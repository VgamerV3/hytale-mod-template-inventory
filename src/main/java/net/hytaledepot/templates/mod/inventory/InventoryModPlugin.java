...
public final class InventoryModPlugin extends JavaPlugin {
  ...
  private final InventoryModTemplate service = new InventoryModTemplate();
  private final AtomicLong heartbeatTicks = new AtomicLong();
  private final ScheduledExecutorService scheduler =
      Executors.newSingleThreadScheduledExecutor(
          runnable -> {
            Thread thread = new Thread(runnable, "hd-inventory-mod-worker");
  ...
  public InventoryModPlugin(JavaPluginInit init) {
    super(init);
  }
  ...
  protected void setup() {
    ...
    getCommandRegistry().registerCommand(new InventoryModStatusCommand());
    getCommandRegistry().registerCommand(new InventoryModDemoCommand());
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              try {
                ...
                if (tick % 60 == 0) {
                  getLogger().atInfo().log("[InventoryMod] heartbeat=%d", tick);
                }
              } catch (Exception exception) {
                lifecycle = Lifecycle.FAILED;
                service.incrementErrorCount();
                getLogger().atInfo().log("[InventoryMod] heartbeat failed: %s", exception.getMessage());
              }
            },
            1,
            1,
  ...
  private final class InventoryModStatusCommand extends CommandBase {
    private InventoryModStatusCommand() {
      super("hdinventorymodstatus", "Shows runtime status for InventoryModPlugin.");
  ...
  private final class InventoryModDemoCommand extends CommandBase {
    private InventoryModDemoCommand() {
      super("hdinventorymoddemo", "Runs a demo action for InventoryModPlugin.");
    setAllowsExtraArguments(true);
      this.setPermissionGroup(GameMode.Adventure);
    }
